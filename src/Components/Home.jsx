...
const Hero = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      impostaIndiceImmagineCorrente(prevIndex => (prevIndex + 1) % immagini.length);
    }, 8000);
    ...
    <div className='mb-32 mt-10 flex justify-center items-center' id="home">
      <div className='w-auto mx-auto text-center flex flex-col md:flex-row mt-20'>
        ...
        <div className='md:w-1/2 md:pl-8 mt-3 mr-3 ml-3'>
          <h1 className='text-[#4D4D4D] md:text-5xl sm:text-5xl text-2xl font-bold md:py-4 mb-5 mt-5 ml-5 mr-5'>
            {testi[indiceImmagineCorrente]}
          </h1>
          <p className='md:text-lg text-base font-bold text-[#717171] ml-5 mr-5 mt-3'>
            {indiceImmagineCorrente === 0 && 'Porta il controllo dei tuoi dispositivi direttamente nelle tue mani. Grazie alla nostra app'}
            {indiceImmagineCorrente === 1 && 'Connetti i tuoi dispositivi al tuo profilo per avere accesso istantaneo a tutte le tue preferenze e impostazioni.'}
            {indiceImmagineCorrente === 2 && 'La nostra app ti consente di automatizzare le azioni quotidiane dei tuoi dispositivi. Configura facilmente scenari personalizzati.'}
          </p>
        </div>
      </div>
    </div>
