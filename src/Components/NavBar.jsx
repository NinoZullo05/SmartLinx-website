...
const Navbar = () => {
  ...
  return (
    <nav className="fixed w-full z-20 top-0 backdrop-filter backdrop-blur-lg bg-opacity-15 bg-inherit">
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        >
          <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border rounded-lg md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 dark:border-gray-700">
            {["home", "servizi", "feature", "download", "faq"].map(
              (section) => (
                <li key={section}>
                  <Link
                    to={section}
                    smooth={true}
                    activeClass="active"
                    className={`block py-2 px-3 text-gray-700 dark:text-gray-300 rounded md:p-0 md:text-[#9E5CF6] cursor-pointer ${
                      activeSection === section ? "bg-[#9E5CF6]" : ""
                    }`}
